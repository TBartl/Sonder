...
public class IslandGen : NetworkBehaviour {
    ...
    [SyncVar] [HideInInspector] public float radius;
    [SyncVar] [HideInInspector] public int size;
    [SyncVar] public int rings;
    float timeRecalc;
    ...
    [SyncVar] [HideInInspector] public int iStatsIndex;
    ...
    public void SetFar(bool isFar) {
        if (isFar != farHolder.gameObject.activeSelf) {
            ...
            foreach (ParticleSystem pSystem in closeHolder.GetComponentsInChildren<ParticleSystem>()) {
                ParticleSystem.EmissionModule pEmission = pSystem.emission;
                pEmission.enabled = !isFar;
            }
