...
public class PlayerMain : LiveEntity {
	...
	[HideInInspector] public PlayerIslandRadar islandRadar;
	...
	void Awake() {
		...
		islandRadar = GetComponent<PlayerIslandRadar> (); 
	...
	[ClientRpc] public void RpcApplyDamage(float damage, float dazeMovementTime, float dazeActionTime, float force, Vector3 direction) {
	...
	[Command] void CmdUpdateWeapon(ItemInstance i) {
		weaponInst = i;
		RpcUpdateWeapon (i);}

	[ClientRpc] void RpcUpdateWeapon(ItemInstance i) {
		weaponInst = i;	
		animationController.AttachWeapon ();}

	[Command] void CmdUpdateArmor(ItemInstance i) {
		armorInst = i;
		RpcUpdateArmor (i);}
	[ClientRpc] void RpcUpdateArmor(ItemInstance i) {
		armorInst = i;	
		animationController.AttachArmor ();}

	[Command] void CmdUpdateArtifact(ItemInstance i) {
		artifactInst = i;
		RpcUpdateArtifact (i);}
	[ClientRpc] void RpcUpdateArtifact(ItemInstance i) {
		artifactInst = i;	
		animationController.AttachArtifact ();}
	...
	void CmdDropItem(ItemInstance itemInst, GameObject islandGameObject) {
		...
		temp.GetComponent<ParticleSystem> ().enableEmission = true;
